...
const Chamada = ({item, toggle}) => {
    ...
    const encerrarChamada = () => {
        fetch('https://api.box3.work/api/Telefone/31c46c8c-cba4-445a-8710-cdfa7432efcf/' + idLigacao, 
        {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
    ...
    const telaFim = () => {
        return (
            ...
            <textarea id="assunto" name="assunto" onChange={handleChange}></textarea>
            <button onClick={encerrarChamada}>Finalizar</button>
            </>
        )
    }
    
    useEffect(() => {iniciarChamada()}, [])
    ...
    return (
        <>
        {estadoChamada? telaInicio(): telaFim()}
        </>
    )
}
